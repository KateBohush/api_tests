...
/**
 * Created by Yarovlav_Ubozhenko on 7/26/2017.
 */
public class CustomEasyTestRunner extends BlockJUnit4ClassRunner {

    /**
     * An instance of logger associated with the test framework.
     */
    private final Logger LOG = LoggerFactory.getLogger(CustomEasyTestRunner.class);
     ...
     */
    public CustomEasyTestRunner(Class<?> klass) throws InitializationError {
    ...
    protected Statement methodInvoker(FrameworkMethod method, Object testInstance) {
        ...
        if(((EasyFrameworkMethod)method).getChildMethods()!= null && !((EasyFrameworkMethod)method).getChildMethods().isEmpty()) {
            for(EasyFrameworkMethod childMethod : ((EasyFrameworkMethod)method).getChildMethods()) {
                try {
                    given = (String)childMethod.getTestData().get("testcase");
